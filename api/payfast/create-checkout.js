...
function buildOrdered(params) {
  const obj = {};
  Object.keys(params)
    .filter(k => params[k] !== '' && params[k] != null)
    .sort()
    .forEach(k => (obj[k] = String(params[k])));
  return obj;
}
function qsPlus(obj) {
  const u = new URLSearchParams();
  for (const [k, v] of Object.entries(obj)) u.append(k, v);
  return u.toString(); // spaces => +
}
function qsPct(obj) {
  return Object.entries(obj)
    .map(([k, v]) => `${k}=${enc(v)}`) // spaces => %20
    .join('&');
}
function sign(baseStr, passphrase) {
  const s = passphrase ? `${baseStr}&passphrase=${encodeURIComponent(passphrase)}` : baseStr;
...
module.exports = async (req, res) => {
  ...
  // default: use PLUS (matches most PF examples)
  if (req.query && String(req.query.debug) === '1') {
    return res.json({
      ok: true, env: forceSandbox ? 'sandbox' : 'live',
      plus: { q: qPlus, sigStr: PASS ? `${qPlus}&passphrase=${encodeURIComponent(PASS)}` : qPlus, signature: sigPlus, url: urlPlus },
      pct:  { q: qPct,  sigStr: PASS ? `${qPct}&passphrase=${encodeURIComponent(PASS)}`  : qPct,  signature: sigPct,  url: urlPct  }
    });
  }

  // Use PLUS by default. If PF rejects it, switch to PCT.
