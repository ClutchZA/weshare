...
function qsPlus(obj) {                 // application/x-www-form-urlencoded (spaces => +)
  const u = new URLSearchParams();
  for (const [k,v] of Object.entries(obj)) u.append(k, v);
  return u.toString();
}
function sign(base, pass) {            // add passphrase only in LIVE
...
module.exports = async (req, res) => {
  ...
  // minimal safe set (we can add extras later)
  ...
  const signature = sign(toSign, PASS);      // passphrase only in LIVE
