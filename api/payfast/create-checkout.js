...
module.exports = async (req, res) => {
  ...
  // If keys missing -> return MOCK url so you can test
  if (!MID || !MK) {
    ...
    return res.status(200).json({ mock: true, url: mock });
  }
  ...
  const params = {
    merchant_id: MID,
    merchant_key: MK,
    return_url: process.env.RETURN_URL || `${base}/success.html`,
    cancel_url: process.env.CANCEL_URL || `${base}/cancel.html`,
    notify_url: process.env.NOTIFY_URL || `${base}/api/payfast/ipn`,
    name_first: 'WeShare',
    name_last: 'Customer',
    email_address: email,
    amount,
  ...
  return res.status(200).json({ mock: false, url });
};